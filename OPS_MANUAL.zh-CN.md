...
| 2026-08-08 | 本 PR（P0 外链建设 + P1 品牌词 + P3 月度复测） | **加速 AI 语料覆盖三件事落地**：① **P0 外链建设/收录加速**——新增 `public/<IndexNowKey>.txt` 域名验证密钥 + `scripts/indexnow-submit.py`（从 sitemap 收集 URL → POST api.indexnow.org，Bing/百度/Yandex 数小时抓取），并在 `.cnb.yml` `main: push` 流水线增加 `indexnow` stage 自动推送前 200 个核心 URL；新增 `geo/外链建设台账.md`（含导航站/社区提交 SOP 与进度记录）；② **P1 品牌词破「黎明撞名」**——`AI_OPERATIONS.zh-CN.md` 新增 §3.1「品牌词标题规范（黎明岛 + 领域词）」，账号/指南/分类/新闻标题统一带 `黎明岛`（en `Dawn Island`），修复唯一残留 `accounts/fireship.md` 标题缺品牌词；③ **P3 月度 AI 可见性复测**——`.cnb.yml` crontab 月度任务新增 `ai-visibility` stage（随 GSC 审计一起跑 `check_ai_visibility.py`，CC 语料覆盖 + 360/必应品牌词露出，结果落 `geo/monthly/ai-visibility-YYYY-MM.md`），目标 2 个月内 CC 覆盖新站 ≥ 1 批次；`gsc/GSC_跟踪与关键词校准手册.md` 新增 §7 复测说明 | 运营流程 + 构建配置 + 内容规范；IndexNow key 部署后线上可访问；不影响页面/路由；品牌词标题规范自 2026-08-08 起对所有新增内容强制生效 |
| 2026-08-09 | 本 PR（Clarity 数据导出） | **Microsoft Clarity 数据导出自动化（行为侧监测补充，与 GSC 搜索侧互补）**：① 新增 `scripts/clarity-export.py`——调用 Clarity Data Export API（`GET clarity.ms/export-data/api/v1/project-live-insights`，JWT Bearer 认证）导出最近 1-3 天会话数据（URL 热门页面 + Source 来源渠道维度），输出原始 JSON + 扁平化 CSV + 汇总 JSON；② `.cnb.yml` 新增 `clarity-export` 流水线（复用锚点）：crontab 每月 1 日 02:45 自动执行 + `api_trigger` 手动一键触发；密钥走密钥仓库 `aoobee-secrets/limingdao-clarity-bot.yml` imports 注入 → `CLARITY_API_TOKEN`；③ **注意 Clarity API 配额：每项目每天最多 10 次请求、只能回溯 1~3 天**，故与 GSC 分开定时避免超限；Clarity 适合看实时/近 3 天行为，28 天趋势仍以 GSC 为准；④ **密钥文件规范验证**：实测无扩展名文件 `limingdao-clarity-bot` 可被 imports 读取但变量无法注入，必须带 `.yml` 扩展名且为标准冒号 YAML（`CLARITY_API_TOKEN: <token>`，**英文冒号+空格+半角值**）——与 `limingdao-cnb-sync` 处理一致；⑤ **防尾注加固**：严禁在密钥值后带 `///` 尾注（如 `CLARITY_API_TOKEN: xxx ///这是注释`），否则尾注会被一并注入 token 值导致 Clarity 鉴权失败；`scripts/clarity-export.py` 的 `get_token()` 已加入防御性剥离（检测到 `///` 自动截断并告警），密钥文件须改为标准格式 | 运营流程；脚本与配置不影响线上页面/路由/构建；数据落 `clarity/export/`；打通「Clarity 行为数据 → 仓库归档」闭环，可回答「是否有真人访问 / 会话来源 / 热门页面」 |
| 2026-08-10 | 本 PR（Clarity Source 修复 + 首页 UX + zh 活跃度 + P0 加码） | **基于 Clarity/GSC 双数据审计的四项落地（issue #18）**：① **P1 Source 维度导出修复**——`scripts/clarity-export.py` 的 `flatten()` 原来忽略 `dim` 参数导致 URL/Source 两个 CSV 输出同一份全量数据；现按 dim 过滤（URL 维度保留 `Url` 非空行、Source 维度保留 `Source` 非空行、交叉行两维度均保留），实测真实数据 URL 1490 行 / Source 171 行，真正差异化；② **P2 首页 UX 微调**——针对 Clarity 死点击 2 + 快速返回 3 全集中 `/zh` 首页（`pages/[lang]/index.astro`）：新增**锚点目录**（本页目录 nav + section id + scroll-margin）、**返回顶部**按钮（平滑回顶脚本）、为长首页各 section 补充锚点 id，减少快速返回与死点击；③ **P2 zh 活跃度提升**——首页中段新增「相关指南」面板、锚点目录等互动元素提升中文页停留与互动；④ **P0 搜索引擎获客继续加码**——`scripts/indexnow-submit.py` 的 `PRIORITY_PATHS` 补充平台页/英文版/热门领域/工具分类等 22 个高价值路径（下次 push 自动推送）；品牌词 SEO 加码：10 个缺 seo 的分类页补齐 `title_zh/title_en/meta_description`（「黎明岛 + 领域词」长尾，实测 `<title>` 生效如「AI 写作工具推荐｜黎明岛」）、3 条 news 标题补品牌词；`geo/外链建设台账.md` 登记本次进度 | 全站；P1 影响 Clarity 导出 CSV 维度；P2 影响首页结构/样式（新增 home-toc/back-to-top/相关指南）；P0 影响 IndexNow 推送范围与分类/news 页 SEO 标题；构建 2672 页通过 |
