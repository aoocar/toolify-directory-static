---
slug: pangmen-ppt
profileUrl: https://space.bilibili.com/395619513
avatar: 📑
platform: bilibili
platformId: '395619513'
verified: true
categories:
  - office
tags:
  - PPT教程
  - 职场技能
  - 设计排版
contentStyle:
  - 系统课程
  - 技巧短视频
monetization: courses
featured: true
followerCount: 1490000
avgEngagement: 0
contentFrequency: weekly
growthRate: 0
publishedAt: '2026-08-01'
updatedAt: '2026-07-31'
name:
  en: PangMenZuoDao PPT
  zh: 旁门左道PPT
tagline:
  en: Bilibili PPT design & workplace tutorial creator
  zh: B站 PPT 设计与职场教程创作者
description:
  en: >-
    PangMenZuoDao PPT (Ya Chen) is a Bilibili renowned creator and course instructor focused on PPT
    design and workplace efficiency. She offers a full learning path from zero to advanced plus a
    template library. Cited at about 1.49M Bilibili followers.
  zh: 旁门左道PPT（雅晨）是B站知名UP主、课堂优质讲师，专注 PPT 设计与职场效率，提供从零基础到进阶的完整课程与模板库。公开资料提及 B站粉丝约149万。
seo:
  primary_keyword: PangMenZuoDao PPT AI creator
  secondary_keywords:
    - PPT教程
    - 职场技能
    - 设计排版
  search_intent: informational
  title_zh: 旁门左道PPT - AI 创作者介绍
  title_en: PangMenZuoDao PPT - AI Creator Directory
  meta_description_zh: ''
  meta_description_en: ''
geo:
  answer_summary_zh: 旁门左道PPT（雅晨）是B站知名UP主、课堂优质讲师，专注 PPT 设计与职场效率，提供从零基础到进阶的完整课程与模板库。公开资料提及 B站粉丝约149万。
  answer_summary_en: >-
    PangMenZuoDao PPT (Ya Chen) is a Bilibili renowned creator and course instructor focused on PPT
    design and workplace efficiency. She offers a full learning path from zero to advanced plus a
    template library. Cited at about 1.49M Bilibili followers.
  facts_zh:
    - 平台: B站（Bilibili）
    - 领域: 办公
    - 粉丝量: 约149万
    - 内容方向: PPT教程、职场技能、设计排版
  facts_en:
    - Platform: Bilibili
    - Niche: Office & Productivity
    - Followers: ~1.49M
  faq_zh:
    - 这个账号主要讲什么？: B站 PPT 设计与职场教程创作者
    - 适合谁关注？: 对办公感兴趣、希望系统了解并上手实践的读者。
    - 在哪里能看到？: B站（Bilibili）（主页见本页外链）。
    - 粉丝规模？: 约149万（公开可查，仅供参考）。
  faq_en:
    - What does this account cover?: Bilibili PPT design & workplace tutorial creator
    - Who is it for?: Readers interested in Office & Productivity who want a structured, hands-on path.
    - Where to watch?: Bilibili.
    - How big is the audience?: ~1.49M (publicly reported, for reference).
---
