---
slug: gityuanbao
profileUrl: https://space.bilibili.com/38061207/
avatar: 🕵️
platform: bilibili
platformId: '38061207'
verified: true
categories:
  - ai-content
tags:
  - AI鉴定
  - 反诈科普
  - 大模型测评
  - GEO
contentStyle:
  - 热点鉴定
  - 技术拆解
  - 公益导向
monetization: ads
featured: false
followerCount: 1020000
avgEngagement: 0
contentFrequency: weekly
growthRate: 0
publishedAt: '2020-01-01'
updatedAt: '2026-07-31'
name:
  en: Git Yuanbao
  zh: Git源宝
tagline:
  en: Exposing AI scams and teaching people to spot fake AI
  zh: 拆解网络热门 AI，教普通人识破 AI 骗局
description:
  en: >-
    Dong Wei (Git Yuanbao) is a Bilibili creator famous for his 'viral AI fact-check' series,
    debunking overhyped or fraudulent AI content and teaching ordinary users — especially the
    elderly — how to recognize deepfakes, voice clones and GEO grey-markets. He builds open tools to
    help the 'left-behind' use AI safely.
  zh: >-
    董纬（Git 源宝）以「网络热门 AI 鉴定」系列闻名，拆解被过度炒作或带有欺诈性质的 AI 内容，教普通人——尤其是老年人——识别换脸、声音克隆与 GEO
    灰产。他还开发开源工具，帮助「慢下来的人」安全地用上 AI。
seo:
  primary_keyword: Git Yuanbao AI creator
  secondary_keywords:
    - AI鉴定
    - 反诈科普
    - 大模型测评
    - GEO
  search_intent: informational
  title_zh: Git源宝 - AI 创作者介绍
  title_en: Git Yuanbao - AI Creator Directory
  meta_description_zh: ''
  meta_description_en: ''
geo:
  answer_summary_zh: >-
    董纬（Git 源宝）以「网络热门 AI 鉴定」系列闻名，拆解被过度炒作或带有欺诈性质的 AI 内容，教普通人——尤其是老年人——识别换脸、声音克隆与 GEO
    灰产。他还开发开源工具，帮助「慢下来的人」安全地用上 AI。
  answer_summary_en: >-
    Dong Wei (Git Yuanbao) is a Bilibili creator famous for his 'viral AI fact-check' series,
    debunking overhyped or fraudulent AI content and teaching ordinary users — especially the
    elderly — how to recognize deepfakes, voice clones and GEO grey-markets. He builds open tools to
    help the 'left-behind' use AI safely.
  facts_zh:
    - 平台: B站（Bilibili）
    - 领域: AI 人工智能
    - 粉丝量: 约102万
    - 内容方向: AI鉴定、反诈科普、大模型测评
  facts_en:
    - Platform: Bilibili
    - Niche: AI & Artificial Intelligence
    - Followers: ~1.02M
  faq_zh:
    - 这个账号主要讲什么？: 拆解网络热门 AI，教普通人识破 AI 骗局
    - 适合谁关注？: 对AI 人工智能感兴趣、希望系统了解并上手实践的读者。
    - 在哪里能看到？: B站（Bilibili）（主页见本页外链）。
    - 粉丝规模？: 约102万（公开可查，仅供参考）。
  faq_en:
    - What does this account cover?: Exposing AI scams and teaching people to spot fake AI
    - Who is it for?: Readers interested in AI & Artificial Intelligence who want a structured, hands-on path.
    - Where to watch?: Bilibili.
    - How big is the audience?: ~1.02M (publicly reported, for reference).
---
